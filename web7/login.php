...
function change_pass($db){session_start(); 
  ...
  $stmt = $db->prepare("SELECT * FROM login_pass WHERE login = ?");
    $stmt->execute(array(
      $login_ch
    ));
  $user = $stmt->fetch();
  if (password_verify($old_pass, $user['pass'])) {
    $stmt = $db->prepare("UPDATE login_pass SET pass = ? WHERE login = ?");
      $stmt -> execute(array(
...
// Иначе, если запрос был методом POST, т.е. нужно сделать авторизацию с записью логина в сессию.
  ...
  $db = new PDO('mysql:host=localhost;dbname=u67298', $user, $pass, array(
